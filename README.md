...
The upload functions require the Cloudflare Pages R2 binding `CHIPS_MEDIA` mapped to the `chips-media` bucket. Audio files are stored under `audio/{actor_id}/{sample_id}.{extension}` and submission metadata is stored under `submissions/`.
